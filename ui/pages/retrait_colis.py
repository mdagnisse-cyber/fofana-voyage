...
class RetraitColisPage(BoxLayout):
    ...
    def _generer_ouvrir_recu(self, c: dict):
        try:
            from utils.pdf_generator import generer_recu_retrait
            from utils.file_opener import ouvrir_fichier
            ...
            chemin = generer_recu_retrait(data)
            ok_pdf, msg_pdf = ouvrir_fichier(chemin)
            if not ok_pdf:
                toast(msg_pdf, "info")
        except Exception as e:
            toast(f"Reçu PDF : erreur ({e})", "erreur")
    ...
    def _valider_otp(self, *a):
        ...
        if ok:
            toast("Retrait confirmé avec succès !", "succes")
            self._etape4()
        else:
