...
class ParametresPage(BoxLayout):
    ...
    def _charger(self):
        try:
            ...
            res = {r['cle']: r['valeur'] for r in cur.fetchall()}
            conn.close()
            return res
        except Exception:
            return {}
    ...
    def _build(self):
        ...
        # ── Sauvegarde BDD ──────────────────────────────────────────
        _section_header(body, 'SAUVEGARDE BASE DE DONNEES', '#3498DB')

        try:
            taille = os.path.getsize(DB_PATH)
            info_bdd = f"Taille : {taille:,} octets"
        except Exception:
            info_bdd = "Fichier base de donnees"
        ...
        btn_bak = Button(
            text='Sauvegarder la base maintenant',
            font_size=dp(12), bold=True,
            background_color=get_color_from_hex('#3498DB'),
            background_normal='', color=(1,1,1,1),
            size_hint_y=None, height=dp(42))
        btn_bak.bind(on_press=self._sauvegarder_bdd)
        body.add_widget(btn_bak)
        ...
        try:
            conn = get_connection(); cur = conn.cursor()
            cur.execute("""
                SELECT s.action, s.created_at,
                       u.prenom||' '||u.nom AS agent
                FROM sessions s
    ...
    def _sauvegarder_bdd(self, *a):
        try:
            bkp_dir = os.path.join(BASE_DIR, 'database', 'backups')
            os.makedirs(bkp_dir, exist_ok=True)
            nom  = f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
            dest = os.path.join(bkp_dir, nom)
            shutil.copy2(DB_PATH, dest)
            toast(f'Sauvegarde OK : {nom}', 'succes')
        except Exception as e:
            toast(f'Erreur sauvegarde : {e}', 'erreur')
