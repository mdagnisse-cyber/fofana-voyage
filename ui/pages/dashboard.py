...
class DashboardPage(BoxLayout):
    ...
    def _build(self):
        ...
        if self._erreur_stats:
            contenu.add_widget(Label(
                text=f"Statistiques indisponibles : {self._erreur_stats[:80]}",
                font_size=dp(10),
    ...
    def _stats(self) -> dict:
        ...
        try:
            s.update(rpc("dashboard_stats"))
        except Exception as e:
            self._erreur_stats = str(e)
        return s

    def _derniers_colis(self) -> list:
        try:
            return rpc("derniers_colis", {"p_limite": 6}) or []
        except Exception:
            return []
