...
class GestionAgencesPage(BoxLayout):
    ...
    def _charger(self):
        ...
        try:
            ...
            cur.execute("""SELECT a.*,
                (SELECT COUNT(*) FROM colis c WHERE c.agence_depart_id=a.id
                    OR c.agence_arrivee_id=a.id) AS nb_colis
                FROM agences a ORDER BY a.ville""")
            agences = [dict(r) for r in cur.fetchall()]
            conn.close()
    ...
    def _form(self, agence=None):
        ...
        def _sauv(*a):
            ...
            try:
                conn = get_connection(); cur = conn.cursor()
                cur.execute("""INSERT INTO agences
                    (code,nom,ville,adresse,telephone)
                    VALUES(?,?,?,?,?)""",
                    (c_code.texte.upper(), c_nom.texte, c_vil.texte,
                     c_adr.texte, c_tel.texte))
