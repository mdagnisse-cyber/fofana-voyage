...
from database.db_manager import rpc, update
...
class AlertesPage(BoxLayout):
    ...
    def _recharger(self):
        ...
        try:
            alertes = rpc("liste_alertes_actives") or []
    ...
    def _resoudre(self, alerte_id):
        try:
            update("alertes", filters={"id": f"eq.{alerte_id}"}, data={
                "resolue": 1,
                "resolue_par": auth.user_id,
                "resolue_at": datetime.now().isoformat(),
            })
            self._recharger()
            toast("Alerte résolue avec succès.", "succes")
        except Exception as e:
            toast(f"Erreur : impossible de résoudre l'alerte ({e})", "erreur")
