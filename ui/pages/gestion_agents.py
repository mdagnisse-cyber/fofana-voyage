...
class GestionAgentsPage(BoxLayout):
    ...
    def _charger(self):
        ...
        try:
            ...
            cur.execute("""SELECT u.*, a.nom as agence_nom
                FROM utilisateurs u LEFT JOIN agences a ON u.agence_id=a.id
                ORDER BY u.nom""")
    ...
    def _form_agent(self, agent=None):
        ...
        try:
            conn = get_connection(); cur = conn.cursor()
            cur.execute("SELECT id, code, nom FROM agences WHERE est_active=1")
