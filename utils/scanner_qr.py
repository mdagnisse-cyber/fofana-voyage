...
# ─── Android : scanner natif ZXing integre a l'APK (pas d'appli tierce) ──────

_REQUEST_CODE_SCAN = 4351


def _lancer_scan_android(callback_succes, callback_echec=None):
    """
    Lance l'activite de scan ZXing embarquee dans l'APK (Gradle dependency
    com.journeyapps:zxing-android-embedded). Aucune installation externe
    requise par l'utilisateur : la bibliotheque de scan est deja incluse
    dans l'application.
    """
    try:
        ...
        intent = Intent(current_activity, CaptureActivity)
        intent.putExtra('SCAN_MODE', 'QR_CODE_MODE')

        def on_activity_result(request_code, result_code, intent_result):
            if request_code != _REQUEST_CODE_SCAN:
                return
            activity.unbind(on_activity_result=on_activity_result)
            try:
                if intent_result is None:
                    if callback_echec: callback_echec("Scan annule.")
                    return
                contenu = intent_result.getStringExtra('SCAN_RESULT')
                if contenu:
                    callback_succes(contenu)
                elif callback_echec:
                    callback_echec("Aucun code detecte.")
            except Exception as e:
                if callback_echec: callback_echec(str(e))

        activity.bind(on_activity_result=on_activity_result)
        current_activity.startActivityForResult(intent, _REQUEST_CODE_SCAN)

    except Exception as e:
        if callback_echec:
            callback_echec(f"Scanner natif indisponible : {e}")
...
class ScannerWidget(BoxLayout):
    ...
    def _build(self):
        ...
        if platform == 'android':
            # Sur telephone : bouton qui lance le scanner natif plein ecran
            # (bibliotheque ZXing integree a l'APK, aucune appli tierce requise)
            btn_scan = Button(
                text='OUVRIR LA CAMERA POUR SCANNER',
                font_size=dp(13), bold=True,
                background_color=get_color_from_hex('#27AE60'),
                background_normal='', color=(1,1,1,1),
                size_hint_y=None, height=dp(64))
            btn_scan.bind(on_press=lambda a: self._lancer_scan_natif())
            self.add_widget(btn_scan)
        else:
            # Sur PC : apercu camera integre directement dans l'ecran
    ...
    def _lancer_scan_natif(self):
        _lancer_scan_android(
            callback_succes=self._traiter,
            callback_echec=lambda msg: self._set_statut(msg, '#C0392B'))
