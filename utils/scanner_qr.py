...
class CameraScanWidget(BoxLayout):
    ...
    def _analyser(self, dt):
        ...
        try:
            ...
            taille = tex.size
            mode  = {'rgba': 'RGBA', 'rgb': 'RGB',
                     'luminance': 'L', 'bgr': 'RGB'}.get(tex.colorfmt, 'RGBA')

            img = Image.frombytes(mode, taille, tex.pixels)
            img = img.convert('L')                       # niveaux de gris
            img = img.transpose(Image.FLIP_TOP_BOTTOM)    # orientation camera Kivy

            resultats = decode(img)
            if resultats:
                data = resultats[0].data.decode('utf-8', errors='ignore').strip()
                if data and data != self._derniere_lecture:
                    self._derniere_lecture = data
                    self._arreter()
                    self.callback_succes(data)

        except ModuleNotFoundError:
            # pyzbar absent (ne devrait pas arriver si buildozer.spec est a jour)
            self._arreter()
